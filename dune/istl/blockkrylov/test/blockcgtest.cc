...
TestSuite test(){
  ...
  config["maxit"] = "1000";
  config["verbose"] = "1";
  BlockCG<BVector, P> solver0(fop, prec, config);
  ...
  tsuite.check(Simd::allTrue(100.0 * b0.two_norm() > config.get<double>("reduction")*def0), "convergence test failed!");
  return tsuite;
}
...
int main(int argc, char** argv)
{
  ...
  prec = std::make_shared<SeqSSOR<BCRSMat,BVector,BVector>>(mat, 1,1.0);
